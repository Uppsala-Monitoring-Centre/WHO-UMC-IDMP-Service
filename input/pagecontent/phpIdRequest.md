...
Upon a successful request, a PhPID is generated and an (APD) resource representing it will be available, referenced in the output section of the Task. The UMC Product ID of the record in WHODrug, UMC's drug dictionary, corresponding to the provided MPD will also be available in the output.   
...
Repeat step (4) for all tasks every 24 hours until the status is 'completed', or `GET` only validated tasks by using the parameter status=completed in the API call.
5. When finished the `GET` call will display status 'completed' and the body of the `GET` will contain the `Task` with a reference to the generated `AdministrableProductDefinition` (APD) resource (including a PhPID) in the output section as well as a UMC product ID as an identifier.
